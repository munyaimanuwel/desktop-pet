...
| Field | Value |
| --- | --- |
| **Title** | Desktop Pet v2 ΓÇö a creature that has a life with you |
| **Author** | TBD |
| **Date** | 2026-09-02 |
| **Status** | Approved |
...
| --- | --- | --- |
| Window | `src/electron/main.js` `createWindow()` | 340├ù400, frameless, transparent, `skipTaskbar`, `alwaysOnTop('pop-up-menu')`, click-through with `{ forward: true }`, **global** `CommandOrControl+Shift+P` (same chord as the VS Code / Cursor command palette), tray. Hardware accel disabled on all platforms to fix a Windows blank-surface bug. |
| Character | `src/components/Pet.tsx` + `globals.css` | 200├ù200 SVG blob. CSS state animations, blink timer, facing, walk-bob, sparkles. |
...
| Tick | `src/pet/tick.js`, 30s in main | Idle ΓåÆ sleepy (5 min) ΓåÆ sleeping (15 min). Hunger/energy/happiness decay. Wake greeting. `handleTick` `speak()`s canned lines only ΓÇö it never calls `generateLine`. AI generation happens in `commit()` (event path) and `greetIfNeeded()` (`DAILY_GREETING`). `WAKING_UP` is in `SPECIAL` but is a dead generate path today. |
| Events | `src/pet/events.js` | Pure deltas + 60s cooldown (failures exempt). Level-up at `level * 100` XP. |
| Roam | `src/electron/roam.js` | Every 14ΓÇô40s pick a random X on the **current** display `workArea`. Animate with 32ms `setPosition(x, bounds.y)`. **Y is never chosen.** |
...
1. **Pip does not sit in the world.** Classic desktop pets walk the taskbar. v1 roam explicitly documents "X only ΓÇö the user keeps the height." After a drag, Pip stays at whatever Y they dropped. On a 1440p display that is usually mid-air.
...
4. **The body is a CSS blob.** Readable, charming, but not a walk cycle. Product direction is a 64ΓÇô96px sprite sheet with the same Pip identity.
...
**v2.0 (shippable tag = PR1 + PR3 + PR4 ΓÇö themes #1 and #4)**
...
- Compact pet window (~280├ù240) with settings HUD in-flow above the sprite so feet stay on the floor.
...
4. **Editor integration is a client of the existing event server, not a new IPC path.** The extension, CLI, and hooks all `POST http://127.0.0.1:41823/event`. Pet core stays unaware of VS Code. Rationale: one allowlist, one cooldown, one applyEvent path (`store.applyEvent` ΓåÆ `handleEvent` in main).
...
9. **v2.0 default window stays 340├ù400; compact HUD is a fast-follow, not in the 2.0 tag.** Pinning Y puts feet on the taskbar immediately. Shrinking is polish (and required before perching). Growing the native window without changing renderer layout does **not** free space for settings ΓÇö PR2 must move the pinned HUD in-flow above the sprite.
...
14. **Extension does not own git.** Owner-confirmed: do **not** switch `settings.repoDir` from event `cwd`. Do **not** POST `COMMIT` from the extension. Builds/tests come from whatever folder is open in the editor; git commit polling stays on Settings ΓåÆ Watch repo.
...
| `pet:dragEnd` | Wander on ΓÇö the **sole** "return to floor after a lift" entry |
...
Walk loop (same cadence: 14s + random 0ΓÇô26s, skip if `busy()`):
...
4. Hop: walk to the nearest horizontal edge of the current work area; then **jump** (do not lerp through the bezel ΓÇö monitors are not a continuous desktop in origin space). Place the window at the destination display's floor, X inset 24px from the arriving edge, facing inward. Brief `curious` then `idle`. Persist.
...
**Drag with roam on:** `roam.pause()` on `pet:dragStart` (already). `clampToVisibleArea` AABB-clamps each `dragMove` ΓÇö X and Y both follow the cursor inside the work area. On `pet:dragEnd`, if roam is on, animate Y down to `floorY` of the display under the drop (a 400ΓÇô700ms ease, state `walking` if the horizontal distance is also large). Do not teleport unless `|dy| < 8`. If the user dropped onto another monitor, that monitor is now home. Then `roam.resume()`.
...
- `screen.on('display-added' | 'display-removed' | 'display-metrics-changed', ΓÇª)` ΓåÆ `snapToFloor()` **only if** `settings.roam` and **not** paused for drag.
...
Add to `settings.json` (not pet.json ΓÇö it is a window concern):
...
v2.0 pins the current 340├ù400 window to the floor. The sprite sits at the bottom of `.stage` (`justify-content: flex-end`); ~150px above is transparent click-through. That already looks like "on the taskbar," just a large Pip.

**Do not** implement compact as "grow the BrowserWindow upward and leave HUD `position: absolute; bottom: 8px`." `.hud` lives inside `.hitbox`. Growing the native window only adds transparent space **above** the hitbox; the pinned settings form (`max-height: 340px`) still overlays the pet at the bottom. A 280├ù240 window cannot fit that form; 280├ù420 with HUD still `bottom: 8px` still covers Pip.

PR2 renderer + main contract:

| State | Window size | Renderer layout | Main bounds |
| --- | --- | --- | --- |
| Default | 280├ù240 | Column: speech slot, pet. Compact hover HUD (bars + Feed + Settings) overlays as today, short enough to fit. | Floor-pinned if roam on |
| HUD pinned (settings) | 280├ù420 | **In-flow** column: settings HUD, speech, pet. HUD is not `position: absolute; bottom: 8px`. | `height += 180`, `y -= 180` so feet stay on the floor |
| HUD closed | 280├ù240 | Back to default column | `height -= 180`, `y += 180` |

Implementation: renderer calls `petAPI.setHudPinned(true|false)` (preload ΓåÆ `ipcMain.on('pet:hudPinned')`). Main owns `setBounds`. Do not put window math in React.

Sprite PR later may go to ~220├ù160. Same in-flow + grow-upward rule.
...
- Poll every 500ms: foreground HWND via `koffi` ΓåÆ `user32.GetForegroundWindow` / `GetWindowRect` (or `DwmGetWindowAttribute(DWMWA_EXTENDED_FRAME_BOUNDS)` to skip drop shadows).
- Skip: our HWND, windows with no title bar, fullscreen (`rect` Γëê display `bounds`, not `workArea`), minimized, explorer desktop, windows shorter than 80px.
...
Quote the globs (todayΓÇÖs `src/pet` script already does). Unquoted `src/electron/*.test.js` fails to expand on Windows when the folder has no tests yet.

electron-builder `files` must add `!src/electron/**/*.test.js` so a later glob widen does not pack tests into the asar.

Cases:

- `floorY` on a workArea `{ x: 0, y: 0, width: 1920, height: 1040 }` and height 400 ΓåÆ `y = 640`.
...
After the existing ΓÇ£must be launched with ElectronΓÇ¥ guard (so `app` is valid), at **module load**:
...
app.whenReady().then(() => { /* createWindow, tray, roam, event server ΓÇö as today */ });
```

`second-instance` may close over `mainWindow` / `setHidden` assigned later ΓÇö that is fine. Do not register the lock inside the `whenReady` callback.
...
```34:49:src/electron/events-source.js
  const server = http.createServer((req, res) => {
    ...
    // JSON { type } ΓåÆ ALLOWED ΓåÆ onEvent(type)
  });
  server.listen(PORT, '127.0.0.1', ...)
```

`ALLOWED` is `COMMIT | PUSH | BUILD_SUCCESS | BUILD_FAILURE | TEST_SUCCESS | TEST_FAILURE`. `handleEvent` in main already piles `MULTIPLE_FAILURES` at 3, 6, 9, ΓÇª This is the contract. Do not invent `npm-build` or `jest-fail` types.
...
| Field | Required | Use |
| --- | --- | --- |
| `type` | yes | Existing allowlist. Unknown ΓåÆ 204 + no-op (do not 400; a stale extension should not error-spam). |
...
`GET /health` ΓåÆ `200 { "ok": true }`. **Do not include `name`.** The extension only needs `ok`; plumbing `getName` into `events-source.start()` is extra state for no client. Do not expose `apiKey`, paths, or pet stats.
...
```js
execFile('git', ['rev-list', '--count', '@{u}..HEAD'], { cwd }, ΓÇª) // ahead
execFile('git', ['rev-list', '--count', 'HEAD..@{u}'], { cwd }, ΓÇª) // behind
```

Parse stdout as `parseInt(String(stdout).trim(), 10)`. Non-zero exit (no upstream, not a repo) ΓåÆ skip, same as today's silent HEAD miss.

In-memory (not disk): `{ ahead, behind }`.

Emit `PUSH` only on a transition **from** `ahead > 0` **to** `ahead === 0 && behind === 0`. That is "local had unpushed commits, now we match upstream." A pull that only fast-forwards behindΓåÆ0 with ahead already 0 does not fire. Rebase/force-push can still false-positive; that is acceptable and documented. If there is no upstream, do nothing.
...
If `vscode.env.remoteName` is set, write **once** to an Output channel (`Desktop Pet`: "This window is remote/WSL. Pip lives on your desktop; workspace build tasks may not reach it."). No toast. Do not retry every task. **Do not** also require ΓÇ£not UI kindΓÇ¥: a ui-only VSIX in a WSL window *is* UI-kind, so that extra conjunct would skip the note and leave the original silent no-op. The UI host may still POST if a task event actually arrives (local folders in a mixed workspace). Workspace-host `onDidEndTaskProcess` for remote folders typically never reaches this process in 2.0 ΓÇö that is accepted, not silently unexplained.
...
POST first. Treat `ECONNREFUSED` / `ECONNRESET` / non-204 as quiet (same as CLI failure, without the CLI's stderr). Do **not** GET `/health` before every task ΓÇö that doubles RTT and races with quit. Optional: after a connection failure, skip POSTs for ~30s.

**Classification** (deterministic, no ML). Default is **ignore**. Subscribe with `vscode.tasks.onDidEndTaskProcess((e) => { const task = e.execution && e.execution.task; ΓÇª })` ΓÇö classify `e.execution.task`, not the event wrapper.

Runtime `vscode.Task.group` is a `TaskGroup` with **`id`**: `'build' | 'test' | 'clean' | 'rebuild'` (optional `isDefault`). There is no `.kind` on that object ΓÇö `kind` is the **tasks.json** schema field. Reading `group.kind` yields `undefined` for a default `npm: build` task, so the 2.0 headline path would no-op unless the label matched the test-runner regex.
...
- `{ group: { id: 'build' }, name: 'npm: compile' }` + exit 0 ΓåÆ `BUILD_SUCCESS`
- `{ group: { id: 'test' }, name: 'npm: compile' }` + exit 1 ΓåÆ `TEST_FAILURE` (id wins over the name)
- `{ group: { kind: 'build' }, name: 'webpack' }` ΓåÆ `BUILD_*` (tasks.json-shaped fallback)
- `{ name: 'npm: lint' }` ΓåÆ `null`
- `taskPattern: ""` or invalid `"("` ΓåÆ extra is null; lint still ignored

Log ignored labels at `trace` (`Desktop Pet` output channel). Optionally, if `vscode.tests` and `onDidChangeTestResults` exist, map a failed run ΓåÆ `TEST_FAILURE` once per run id (dedupe against the task event with a 2s window).
...
Git: do **not** emit COMMIT from the extension (owner-confirmed; Key Decision 14). Commit polling stays on Settings ΓåÆ Watch repo. Push: if the built-in Git extension API `onDidPublish` is available, POST `PUSH`; otherwise the ahead/behind poll covers it.

**What it does not do:** no webview, no chat, no reading `pet.json`, no API key.

**Ship path for v2.0:** root script (do **not** pass `--cwd` to vsce ΓÇö that flag is npxΓÇÖs, not vsceΓÇÖs):

```json
"ext:package": "npx --yes --prefix extensions/vscode @vscode/vsce package -o desktop-pet-0.2.0.vsix"
```

(or add `@vscode/vsce` as a **root** `devDependency` and run it with `--prefix extensions/vscode`). The VSIX lands in `extensions/vscode/` and is **not** part of electron-builder `files`. Sideload (`Install from VSIXΓÇª` in VS Code and Cursor). Marketplace publication is v2.1, not required to get builds flowing.
...
Root `README.md` (existing file, small edit in PR1 and PR4 ΓÇö not a new doc site): after 2.0 it must not still say Pip "wanders along your desktop" with builds only via `npm run pet`. PR1 updates wander/shortcut copy. PR4 adds the VSIX sideload paragraph.
...
```
public/pip.png                  # 768├ù96, 8 frames of 96├ù96
public/pip.json
```
...
Identity: same purple (`#6c5ce7`), ears, dark eyes, readable at 64ΓÇô96px. Do not introduce a second character.
...
| `push-gap` | On `PUSH` when `lastPushAt` is ΓëÑ 3 days ago and > 0 | "First push in three days." |
...
- every successful `tick` (30s ΓÇö covers crashes better than quit-only)
...
function maybeSpeakGenerated(event, canned, snapshot) {
  ...
  generateLine({ event, state: snapshot, apiKey: key }).then((line) => {
    ...
    // thinking in place ΓÇö scheduleReturnToIdle would then skip the reaction.
    petState = snapshot;
    if (line) {
      const dayNow = dayKey();
...
- Timeout 3.5s, `store: false`, key only in main ΓÇö unchanged.

`LONG_ABSENCE`: same 36h path as the journal fact, **and** `lastSeenAt > 0`. If a key is present and caps allow, generate; else canned.

`END_OF_DAY`: in `handleTick`, local hour ΓëÑ 18, `dayStats` has any commits/builds/tests, `lastEndOfDay !== today`, idleSeconds < 180 (user is actually here). Fire once through `maybeSpeakGenerated`. Quiet mode: add `LONG_ABSENCE` and `END_OF_DAY` to `QUIET_EVENTS`.
...
- Consider **not** calling `app.disableHardwareAcceleration()` on Darwin if transparency works without it (Windows-only workaround today). Verify; do not guess ΓÇö see Open Questions.
...
flowchart TB
  ...
  subgraph electronMain ["Electron main ΓÇö one process"]
...
Match existing rendererΓåÆmain names (`setHover`, `dragStart`, `setSettings`). `onX` stays mainΓåÆrenderer (`onState`).

PR3 must expose `installHooks` on **both** `src/electron/preload.js` and `src/types/pet-api.d.ts`. Context isolation is on; HUD calling `window.petAPI.installHooks()` is `undefined` without the preload bridge. Main already owns `ipcMain.on('pet:installHooks', ΓÇª)` and the speech-bubble result.
...
v2.1: fields in ┬º4.1. Add each to `createInitialState()` and clamp in `store.normalize()` so missing `journal` is `[]`.

### VS Code contribution

See ┬º2.5. MVP includes `desktopPet.port`, `desktopPet.taskPattern`, and `desktopPet.ping`.
...
Typical `pet.json` today is < 4 KB. Journal cap 60 ├ù ~80 bytes Γëê 5 KB. No migration tool.

### Migration strategy

Two different normalizers ΓÇö do not conflate them.

**`settings.normalize()`** (`src/pet/settings.js`): builds a new object from `defaults()`. **Add every new key here or it vanishes on the next save.** 0.1.0 drops `lastBounds` on rollback ΓÇö acceptable; Pip respawns at the default position.

**`store.normalize()`** (`src/pet/store.js`): `merged = { ...base, ...clone(raw) }` then clamps known fields and returns `merged`. Extra keys (`journal`, `lastSeenAt`, ΓÇª) **survive 0.1.0 load+save**. The real footgun is shipping a field that is `undefined` because it was never added to `createInitialState()` / clamps (e.g. `state.journal.push` throws). Tests in `store.test.js` for every new field: missing ΓåÆ default; garbage ΓåÆ default.
...
- Habitat: 32ms `setPosition` only while walking (~2ΓÇô5s bursts every 14ΓÇô40s) ΓåÆ well under 1% CPU on Windows. Sleeping pet: zero position writes. Drag: AABB clamp only.
- AI: 0ΓÇô4 POSTs/day to `api.x.ai`, 3.5s budget, no storage server-side (`store: false`).
- Log: 1 MB cap.
...
- **Cons:** Full-screen always-on-top windows steal focus, break games, fight DWM, and have already-painful click-through. This app spent v1 fixing a *blank 340├ù400 surface* (`app.disableHardwareAcceleration()`). A fullscreen overlay is a support nightmare. **Rejected.**
...
- **Pros:** One `rev-parse`.
- **Cons:** Pull fast-forwards, fetches, and behind-states look like pushes. **Rejected.** Use `rev-list --count` ahead/behind (see ┬º2.3).
...
3. **Rollback:** reinstall `0.1.0`. `pet.json` extra keys survive 0.1.0 `store.normalize()`. `settings.json` `lastBounds` is dropped by 0.1.0 `settings.normalize()` ΓÇö Pip respawns on the default position. v2.0 does not write journal fields.
4. **v2.1.x** ΓÇö compact if not already, sprite, journal, AI caps, signed/macOS/updater. Each is a separate tag if needed.
5. **Unsigned ΓåÆ signed:** once a cert exists, flip `signAndEditExecutable`, set `CSC_LINK`, keep the same `appId` so userData persists.
...
Weekend sequence if we cut: **PR1 ΓêÑ PR3, then PR4.** PR4 can temporarily target v1 POST-only `{ type }` if PR3 slips.
...
| Floor-snap during drag pins Y | High if mis-implemented | `clampToVisibleArea` stays AABB. Snap only from the table in ┬º1.1. Test: dragMove above the floor does not write `floorY`. |
...
| Compact "grow upward" still covers Pip | High if layout ignored | PR2 must put pinned HUD in-flow above the sprite (┬º1.5). Native height alone is not enough. |
...
| `eventFor` reads `TaskGroup.kind` and misses real builds | High if mis-implemented | Use `task.group.id` (runtime), fall back to `kind` / string. Test `{ group: { id: 'build' }, name: 'npm: compile' }` ΓåÆ `BUILD_*`. |
...
| `EADDRINUSE` ΓÇö two pets | Low (existing) | Keep warn. PR1 calls `requestSingleInstanceLock()` at module load (before `whenReady`), then `second-instance` show/focus, then `whenReady` as today. |
...
| Spoken line lands on `thinking` | Medium | After `generateLine`, restore `petState = snapshot` (then bump AI caps if the line is non-null), persist, then `speak` ΓÇö same as today's `commit()`. |
...
| Pull fast-forward counted as PUSH | High if two-hash compare used | `rev-list --count` ahead/behind; emit only ahead>0 ΓåÆ (0,0). |
...
10. **Watched repo vs "whatever I have open in Cursor":** **Resolved.** Do not switch the watched repo. Do not POST `COMMIT` from the extension. Builds/tests come from whatever folder is open; git commit polling stays on Settings ΓåÆ Watch repo. (Key Decision 14.)
...
Each PR is independently reviewable and mergeable. **Shippable 2.0 tag = PR1 + PR3 + PR4.** PR2 is optional fast-follow (2.0.x or 2.1). PRs 5ΓÇô8 are v2.1. Perch, if ever, is its own PR after PR2 ΓÇö not PR8.
...
PR1 and PR3 are independent ΓÇö they can land in either order, in parallel. Weekend: PR1 ΓêÑ PR3, then PR4.

---

### PR 1 ΓÇö Habitat: walk the work-area floor and hop displays
...
### PR 2 ΓÇö Compact window (optional fast-follow)
...
- **Changes:** Default ~280├ù240. Pinned HUD is in-flow above the pet; main grows height and subtracts Y so feet stay on the floor. CSS pet scales down. Still SVG. May land in 2.0.x or 2.1.

### PR 3 ΓÇö Event protocol, portable hooks, push detection

- **Title:** Let anything local tell Pip about builds ΓÇö safely
- **Files / components:** `src/electron/events-source.js` (`GET /health` `{ok:true}`, body cap on `data` ΓåÆ 413, optional `source`, `port` inject), new `src/electron/events-source.test.js`, `scripts/pet.js` (`source: cli`), new `src/electron/hooks.js` (`http.request` one-liner, not `fetch`), `scripts/install-hooks.js` (delegate), `src/electron/main.js` (IPC `pet:installHooks`, `setRepoDir` unchanged), `src/electron/preload.js` (`installHooks: () => ipcRenderer.send('pet:installHooks')`), `src/types/pet-api.d.ts` (`installHooks: () => void`), `src/components/HUD.tsx` (install-hooks button), `src/electron/log.js` + wire `console.error` sites that matter
- **Depends on:** none
- **Changes:** Health endpoint (no name). Ahead/behind `rev-list --count` emits `PUSH` only on ahead>0 ΓåÆ (0,0). Hooks POST via `http.request` with `source: hook` into `settings.repoDir`. Local log file. CLI remains compatible. Tests: `{type}` ΓåÆ 204; extra `source`/`cwd` ΓåÆ 204; unknown type ΓåÆ 204 and `onEvent` not called; bad JSON ΓåÆ 400; oversize ΓåÆ 413 without parsing; GET `/health` ΓåÆ 200 `{ok:true}`.

### PR 4 ΓÇö VS Code / Cursor extension + 2.0 version bump
...
- **Depends on:** PR3 (health + `source`; can temporarily target v1 POST-only if PR3 slips ΓÇö `{type}` still 204s)
- **Changes:** `onDidEndTaskProcess` ΓåÆ `BUILD_*` / `TEST_*` only when `group.id` / fallback regex / valid `taskPattern` says so; ignore lint/dev/install/watch. Empty/invalid `taskPattern` is null. POST first; quiet on `ECONNREFUSED`. Ping command. One-time Output note whenever `remoteName` is set (do not also require non-UI kind). No webview. VSIX not in the asar. Version 0.2.0 for the 2.0 tag.

### PR 5 ΓÇö Journal facts that become lines
...
### PR 6 ΓÇö Sprite sheet with SVG fallback
...
### PR 7 ΓÇö Rare AI: absence, end of day, hard caps
...
### PR 8 ΓÇö Signed Windows, macOS dmg, quiet GitHub auto-update
...
- **Depends on:** none of 1ΓÇô7 functionally. Certs/repo visibility are Open Questions. **Does not include perch.**
